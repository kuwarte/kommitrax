...
export default function Main() {
  ...
  const handleCreateCommitment = async () => {
    ...
    if (success) {
      setGoal(''); setVerifierAddress(''); setHours('24');
      commitment.loadAllCommitments();
    }
  };
  ...
  const handleSubmitProof = async () => {
    ...
    if (success) {
      setProofText(''); setSelectedCommitment(null);
      commitment.loadAllCommitments();
    }
  };
      ...
      <div className="flex-grow pb-20">
        ...
        {!wallet.connected ? (
          <div className="max-w-7xl mx-auto px-4 py-24 text-center">
            <div className="inline-block border border-black bg-white p-12 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-2xl text-left">
              <h2 className="text-5xl font-black uppercase mb-4 leading-[0.9] tracking-tighter">
                <span className="bg-green-500/30 px-2">Finish the task.</span><br/>
              ...
              <div className="space-y-2 mb-10">
                <div className="flex items-start gap-3">
                  <span className="bg-black/40 text-black px-2 font-bold">STAKE</span>
                  <p className="text-sm font-bold uppercase tracking-tight">Deposit 0.01 ETH as a commitment to your goal.</p>
                </div>
                <div className="flex items-start gap-3">
                  <span className="bg-black/40 text-black px-2 font-bold">RISK</span>
                  <p className="text-sm font-bold uppercase tracking-tight">Failure to provide proof, then funds are forfeited.</p>
                </div>
                <div className="flex items-start gap-3">
                  <span className="bg-green-500/50 text-black px-2 font-bold">REWARD</span>
                  <p className="text-sm font-bold uppercase tracking-tight">Verify your work and reclaim your assets instantly.</p>
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 py-10 sm:px-6 lg:px-8">
              ...
              {(["student", "verifier"] as const).map((tab) => (
                <button
                  ...
                  className={`text-sm font-bold uppercase tracking-widest px-4 py-2 border border-black transition-all ${
                    activeTab === tab ? 'bg-black text-white shadow-[4px_4px_0px_0px_#888]' : 'bg-white text-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]'
                  }`}
                >
                  {tab === 'student' ? 'Student Dashboard' : `Verifier Tasks (${commitment.verifierCommitments?.length || 0})`}
                </button>
              ))}
            </div>
            ...
            {activeTab === 'student' ? (
              <StudentView 
                goal={goal} setGoal={setGoal} verifierAddress={verifierAddress} setVerifierAddress={setVerifierAddress} 
                hours={hours} setHours={setHours} handleCreate={handleCreateCommitment} loading={commitment.loading}
          ...
          <div className="fixed inset-0 bg-white/90 flex items-center justify-center p-4 z-[100]">
            <div className="bg-white border-2 border-black w-full max-w-lg shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
              <div className="bg-black text-white p-4 flex justify-between items-center">
                ...
                <button onClick={() => setSelectedCommitment(null)} className="text-white hover:text-gray-300 font-mono text-xl">X</button>
              </div>
              <div className="p-8">
                <p className="text-sm font-bold mb-4 uppercase text-gray-500">EVIDENCE OF WORK:</p>
                <textarea
                  value={proofText}
                ...
                <div className="flex gap-4">
                  <button onClick={() => setSelectedCommitment(null)} className={`${btnClass} flex-1 bg-white hover:bg-gray-100`}>CANCEL</button>
                  <button onClick={handleSubmitProof} disabled={!proofText} className={`${btnClass} flex-1 bg-blue-400/50 text-black border-black`}>
