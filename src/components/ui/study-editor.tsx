			...
			<div className="max-w-7xl mx-auto px-4 py-10 space-y-8 pb-24">
				<div className="flex flex-col md:flex-row justify-between items-end border-b border-black pb-6 gap-6">
					<div>
						<Link
							href="/"
							className="text-xs font-bold uppercase underline hover:text-zinc-500 mb-2 block"
						>
							To Dashboard
						</Link>
					...
					<div className="flex gap-4">
						{(["flashcards", "quiz"] as const).map((m) => (
							<button
								...
								className={`text-sm font-bold uppercase tracking-widest px-6 py-3 border border-black transition-all ${mode === m ? "bg-black text-white shadow-[4px_4px_0px_0px_#888]" : "bg-white text-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"}`}
							>
								{m}
							</button>
						))}
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
					<div className="lg:col-span-5 h-fit bg-white border border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] sticky top-8">
						...
						<div className="space-y-4">
							...
							<button
								onClick={handleSave}
								className={`${btnClass} w-full py-4 text-black bg-blue-400/50 active:bg-blue-300/50 border border-black`}
							>
					...
					<div className="lg:col-span-7 flex flex-col gap-4">
						<div className="bg-blue-400/50 border border-black p-6 flex justify-between items-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
							<div>
						...
						<div className="max-h-125 overflow-y-auto pr-2 custom-scrollbar border border-black bg-white p-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
							{mode === "flashcards"
								? study.cards.map((c: Flashcard) => (
										<div
											key={c.id}
											className={`group bg-white border-b border-black p-4 flex justify-between items-center hover:bg-gray-200 ${editingId === c.id ? "bg-gray-200 border-l-4 border-l-black" : ""}`}
										>
								...
								: study.quizzes.map((q: QuizItem) => (
										<div
											key={q.id}
											className={`group bg-white border-b-2 border-black p-4 flex justify-between items-center hover:bg-gray-200 ${editingId === q.id ? "bg-gray-200 border-l-8 border-l-black" : ""}`}
										>
											<div className="flex-1 min-w-0 mr-4">
